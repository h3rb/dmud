import processing.core.*; import java.net.*; import java.applet.*; import java.awt.*; import java.awt.image.*; import java.awt.event.*; import java.io.*; import java.net.*; import java.text.*; import java.util.*; import java.util.zip.*; public class dMUD extends PApplet {// dMud by Mad Merv (madmerv@gmail.com) - Created 18 August 2005 with Processing
...
int port= PApplet.toInt(param("port")); 
...
// Visualization
public void draw() {
 background(PApplet.toInt(random(255)));
 updatePixels();
}
...
public String URLencode(String v) {
 ...
 for (int i=0;i<len;i++) {
  char c = v.charAt(i);
  if (PApplet.toInt(c) < 255) {
   if (unsafe.indexOf(c) == -1 && PApplet.toInt(c) > 32 && PApplet.toInt(c) < 123) newStr = newStr + c;
...
public String reverse_direction( String dir ) {
 switch( dir.charAt(0) ) {
...
public class Mud {
 ...
 public void add( Player p ) { players++;
 ...
 // room-specific echo
 public void echo( String msg, int loc ) {
...
public class Mob {
 ...
 public void Mob( ) { }
 public void Mob( int v ) { vnum=v; }

 public void wander() {
  result = loadStrings(WEB+"exits.php?loc="+location);
  mud.echo( name+" has departed.\n\r", location );
  location = PApplet.toInt(result[PApplet.toInt(random(6))]);  // 6="Exit Resolution" u,d,n,s,e,w
...
public class Obj {
 ...
 public void Obj ( ) { }
 public void Obj ( int v ) { vnum=v; }
  
 public void load( String v ) {
  result=loadStrings("obj.php?v="+v); 
  result = split(result[0], "|");
  vnum = PApplet.toInt(v); name = result[1]; type = PApplet.toInt(result[2]); value = PApplet.toInt(result[3]); level = PApplet.toInt(result[4]);
 }
 
 public void load( int v ) {
  result=loadStrings("obj.php?v="+v);
  result= split(result[0], "|");
  vnum = v; name = result[1]; type = PApplet.toInt(result[2]); value = PApplet.toInt(result[3]); level = PApplet.toInt(result[4]);
 }
...
// max URL is 2048 on some browsers, so short variable names
 ...
 // Roll statistics
 private void roll( ) {
  strength    =PApplet.toInt(random(18));
  intelligence=PApplet.toInt(random(18));
  dexterity   =PApplet.toInt(random(18));
  wisdom      =PApplet.toInt(random(18));
  constitution=PApplet.toInt(random(18));
  charisma    =PApplet.toInt(random(18));    
 }
...
//  println(" State: " + state + " \"" + theInput + "\"" );
   ...
   case GET_NAME:
        if ( theInput == "" || theInput.length() < 3 ) theOutput = "Invalid name.  Please enter your name: ";
         ...
         if ( buffer[0].equals("NEW") ) { theOutput = "Create as new? (y/n) "; state = GET_CONFIRM_NEW;  }
          ...
          password    =buffer[1];           wizard      =PApplet.toInt(buffer[2])!=0;
          male        =PApplet.toInt(buffer[3])!=0;   discipline  =buffer[4];
          strength    =PApplet.toInt(buffer[5]);      intelligence=PApplet.toInt(buffer[6]);
          wisdom      =PApplet.toInt(buffer[7]);      dexterity   =PApplet.toInt(buffer[8]);
          constitution=PApplet.toInt(buffer[9]);      charisma    =PApplet.toInt(buffer[10]);
          xp          =PApplet.toInt(buffer[11]);     level       =PApplet.toInt(buffer[12]);
          gold        =PApplet.toInt(buffer[13]);     hp          =PApplet.toInt(buffer[14]);
          mana        =PApplet.toInt(buffer[15]);     damroll     =PApplet.toInt(buffer[16]);
          hitroll     =PApplet.toInt(buffer[17]);     ac          =PApplet.toInt(buffer[18]);
          location    =PApplet.toInt(buffer[19]);     theOutput = "Password: ";
          state = GET_PASSWORD;
         }
        }
     break;
 ...
 public String parser( String cl ) {
...
// Commands:
 ...
 // kill <target>
 public String kill( ) { return ""; }
 
 // tell <target> <message>
 public String tell( String target ) {
 ...
 public String move( String dir ) {
  int dest;
  buffer = loadStrings(WEB+"exit.php?loc="+location+"&dir="+dir); dest=PApplet.toInt(buffer[0]);
 ...
 // teleport <location>
 public String teleport( String dest ) { if (!wizard) return "What?\n\r";
  buffer = loadStrings(WEB+"scene.php?loc="+dest);
  if ( buffer[0].equals("NONE") ) return "That place doesn't exist.\n\r"; 
  location = PApplet.toInt(dest);
  return look();
 }

 // dig
 public String dig( ) { if ( !wizard ) return "What?\n\r";
  buffer = loadStrings(WEB+"dig.php"); debug();
  location = PApplet.toInt(buffer[0]);
  return "Dug.\n\r";
 }

 // connect <direction> <destination>
 public String connect( String dir, String dest ) { if (!wizard) return "What?\n\r";
  String rev_dir = "";
  buffer = loadStrings(WEB+"scene.php?loc="+dest);
 ...
 public String disconnect( String dir ) { if (!wizard) return "What?";
  String rev_dir = "";
  int dest;
  buffer = loadStrings(WEB+"exit.php?loc="+location+"&dir="+dir); dest=PApplet.toInt(buffer[0]);
  buffer = loadStrings(WEB+"scene.php?loc="+dest);
